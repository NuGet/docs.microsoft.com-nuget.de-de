...
- [Paketquellenabschnitte](#package-source-sections): -[Paketquellen](#packagesources)
...
Hinweis: `dependencyVersion` und `repositoryPath` gelten nur für Projekte, in denen `packages.config` verwendet wird. `globalPackagesFolder` gilt nur für Projekte, in denen `project.json` und PackageReference-Formate verwendet werden.
