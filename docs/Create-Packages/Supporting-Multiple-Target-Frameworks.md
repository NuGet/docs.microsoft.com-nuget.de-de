...
*Einzelheiten zur versionsübergreifenden Ausrichtung für .NET Core-Projekte, die NuGet 4.0 oder höher verwenden, finden Sie unter [NuGet pack and restore as MSBuild targets (NuGet-Befehle „pack“ und „restore“ als MSBuild-Ziele)](../schema/msbuild-targets.md).*
...
- [Ordnerstruktur der Frameworkversion](#framework-version-folder-structure)
- [Abgleichen von Assemblyversionen und dem Zielframework in einem Projekt](#matching-assembly-versions-and-the-target-framework-in-a-project)
- [Gruppieren von Assemblys nach Frameworkversion](#grouping-assemblies-by-framework-version)
- [Bestimmen des zu verwendenden NuGet-Ziels](#determining-which-nuget-target-to-use)
- [Inhaltsdateien und PowerShell-Skripts](#content-files-and-powershell-scripts)
...
> [!Warning]
> Änderbare Inhaltsdateien und die Skriptausführung sind nur im Format `packages.config` verfügbar; in `project.json` und den PackagesReference-Formaten sind sie als veraltet gekennzeichnet und sollten nicht für neue Pakete verwendet werden.
