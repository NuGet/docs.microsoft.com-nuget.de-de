...
Ein NuGet-Paket kann problemlos über eine .NET-Klassenbibliothek erstellt und auf nuget.org veröffentlicht werden. Dieser Artikel führt Sie durch den Prozess, bei dem die NuGet-Befehlszeilenschnittstelle (Command-Line Interface, CLI) und Visual Studio verwendet werden.
...
1. Wählen Sie **Create** (Erstellen) aus, geben Sie einen Namen für Ihren Schlüssel an, wählen Sie unter **API Key** (API-Schlüssel) die Option **Select Scopes > Push** (Bereiche auswählen > Push) aus, geben Sie * für **Glob pattern** (Globmuster) ein, und wählen Sie anschließend **Create** (Erstellen) aus.
