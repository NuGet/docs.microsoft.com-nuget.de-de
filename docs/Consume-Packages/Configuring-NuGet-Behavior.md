...
Das Verhalten von NuGet wird durch die Eigenschaften gesteuert, die in einer oder mehreren `NuGet.Config`-Dateien (XML) gesammelt sind, die auf Projekt-, Benutzer- oder Computerebene vorhanden sein kann. Eine globale `NuGetDefaults.Config`-Datei (2.7 und höher) konfiguriert insbesondere Paketquellen. Die Einstellungen gelten für alle Befehle, die in der CLI, der Konsole oder der Benutzeroberfläche des Paket-Managers ausgeführt werden.

In diesem Thema:

- [Speicherorte und Verwendungsmöglichkeiten von NuGet.Config-Dateien](#config-file-locations-and-uses)
- [Ändern der Einstellungen](#changing-config-settings)
- [Anwenden der Einstellungen](#how-settings-are-applied)
- [NuGetDefaults.Config-Datei](#nuget-defaults-file)
...
| --- | --- | --- |
| Projekt | Projektordner oder ein beliebiger anderer Ordner im Stammlaufwerk | Die Einstellungen in einem Projektordner gelten nur für dieses Projekt. Die Einstellungen in einem übergeordneten Ordner, der mehrere Unterordner mit Projekten enthält, gelten für alle Projekte in diesen Unterordnern. |
| Benutzer | Windows: %APPDATA%\NuGet\NuGet.Config<br/>Mac/Linux: ~/.nuget/NuGet.Config | Die Einstellungen gelten für alle Vorgänge, werden jedoch durch sämtliche Einstellungen auf Projektebene überschrieben. Wenn Sie CLI-Befehle verwenden, können Sie eine andere Konfigurationsdatei mithilfe des `-configFile`-Parameters angeben, um sämtliche Einstellungen in der Standarddatei auf Benutzerebene zu ignorieren. |
| Computer | Windows: %ProgramFiles(x86)%\NuGet\Config<br/>Mac/Linux: $XDG_DATA_HOME (typically ~/.local/share) | Die Einstellungen gelten für alle Vorgänge auf dem Computer, werden jedoch durch sämtliche Einstellungen auf Benutzer- oder Projektebene überschrieben. |

Hinweise für frühere Versionen von NuGet:
- In NuGet 3.3 und früher wurde ein `.nuget`-Ordner für projektmappenweite Einstellungen verwendet. Diese Datei wird in NuGet 3.4 und höher nicht verwendet.
...
Bei einer `NuGet.Config`-Datei handelt es sich um eine einfache XML-Textdatei, die wie im Artikel [NuGet Configuration Settings (NuGet-Konfigurationseinstellungen)](../Schema/nuget-config-file.md) beschrieben Schlüssel/Wert-Paare enthält.
...
Datei A (Datei auf Benutzerebene:, %APPDATA%\NuGet\NuGet.Config unter Windows, ~/.nuget/NuGet.Config unter Mac/Linux):
...
Windows: %ProgramFiles(x86)%\NuGet\Config (NuGet 2.7 bis NuGet 3.x: %PROGRAMDATA%\NuGet\NuGetDefaults.Config) Mac/Linux: $XDG_DATA_HOME (üblicherweise ~/.local/share) 

### <a name="nugetdefaultsconfig-settings"></a>Einstellungen von NuGetDefaults.Config

- `packageSources`: Diese Auflistung hat in regulären Konfigurationsdateien die gleiche Bedeutung wie `packageSources` und gibt die Standardquellen in deren bevorzugten Reihenfolge an. Wenn diese Einstellung in `NuGetDefaults.Config` vorhanden ist, verwendet NuGet nicht nuget.org als Standardpaketquelle. Ein Administrator kann daher bei Bedarf verhindern, dass nuget.org verwendet wird, und sicherstellen, dass alle Personen, die diese Datei verwenden, mit den gleichen Quellen arbeiten.
